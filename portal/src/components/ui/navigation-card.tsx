...
export function NavigationCard({ title, description, href, icon }: NavigationCardProps) {
  return (
    <Link
      href={href}
      className={clsx(
        'group flex h-full flex-col justify-between rounded-xl border bg-white p-4 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md dark:border-slate-800 dark:bg-surface-dark'
      )}
    >
      ...
      <span className="mt-3 text-sm font-medium text-sky-600 group-hover:underline dark:text-sky-400">
        Open
      </span>
    </Link>
  );
